...
typedef unsigned long int microinstrucao; // 64 bits, 36 bits utilizados

// Definições de constantes
#define MAX_MICROINSTRUCOES 512
#define TAM_MEMORIA 100000000
#define TAM_INICIALIZACAO 20
...
microinstrucao Armazenamento[MAX_MICROINSTRUCOES];
...
void carregar_microprogram_de_controle() {
    ...
    fread(Armazenamento, sizeof(microinstrucao), MAX_MICROINSTRUCOES, MicroPrograma);
...
void carregar_programa(const char* prog) {
    ...
    fread(Memoria, sizeof(byte), TAM_INICIALIZACAO, Programa); // Lendo os 20 bytes de Inicialização
    fread(&Memoria[0x0401], sizeof(byte), tamanho - TAM_INICIALIZACAO, Programa); // Lendo o Programa
